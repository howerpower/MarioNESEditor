...
export class SingleTileDisplayComponent implements AfterViewInit {
  ...
  private _spriteArray = [0,1,4,5,8,9,12,13]; //a bad way for making sure we draw sprites correctly
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if ("_SelectedTile" in changes) {
      if(this._SelectedTile[0] != undefined){

        // This is where I'm making the changes to
        // load multiple canvases with the entire selected sprite

        //First Clear the entire canvas
        for (var i = 0; i < 16; i++) {
          let ctx = this._canvases[i].getContext("2d");
          ctx.clearRect(0, 0, this._scaledtilesize, this._scaledtilesize);
          this.pushToTiles(null,i);
        }

        //Special Case for Bowser
        if (this._SelectedTile.length == 16) {
          for (var i = 0; i < this._SelectedTile.length; i++) {
            let ctx = this._canvases[i].getContext("2d");
            this.drawTile(ctx, this._SelectedTile[i]);
            this.pushToTiles(this._SelectedTile[i],i);
          }
        }
        else {
          for (var i = 0; i < this._SelectedTile.length; i++) {
            let ctx = this._canvases[this._spriteArray[i]].getContext("2d");
            this.drawTile(ctx, this._SelectedTile[i]);
            this.pushToTiles(this._SelectedTile[i],this._spriteArray[i]);
          }
        }
      }
    }
